...
class CompletionRequest(BaseModel):
    ...
    model: str = Field(description="The identifier of the model designated for completion generation.")
...
class CompletionChoice(BaseModel):
    ...
    index: int = Field(description="The sequential index of this choice within the response list.")
...
class CompletionUsage(BaseModel):
    ...
    prompt_tokens: int = Field(description="The number of tokens constituting the input prompt(s).")
    completion_tokens: int = Field(
        description="The total number of tokens generated across all completion choices."
    )
    total_tokens: int = Field(
        description="The sum of `prompt_tokens` and `completion_tokens`."
    )
...
def generate_completion_id(prefix: str = COMPLETION_ID_PREFIX) -> str:
    """
    Generates a unique identifier string for a completion response, prefixed as specified.

    Args:
        prefix: The string prefix for the generated ID.

    Returns:
        A unique identifier string (e.g., "cmpl-xxxxxxxxxxxxxxxxxxxxxxxxxxxxx").
    """
